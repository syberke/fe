<template>
  <div class="container px-4 py-10 text-black dark:text-white bg-white dark:bg-gray-900 flex">
    
    <div class="w-1/4 bg-white dark:bg-gray-800 p-4 rounded-lg shadow-md h-screen sticky top-0">
      <h2 class="text-lg font-semibold mb-4">Filter</h2>
      <div class="space-y-4">
        <div>
          <label class="block text-sm font-medium">Stock</label>
          <select v-model="filterStock" class="w-full p-2 border rounded bg-gray-100 dark:bg-gray-700 text-black dark:text-white">
        ...
        <div>
          ...
          <select v-model="sortPrice" class="w-full p-2 border rounded bg-gray-100 dark:bg-gray-700 text-black dark:text-white">
    ...
    <div class="w-3/4 pl-6 overflow-auto h-screen">
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
        <div v-for="book in paginatedBooks" :key="book.id" class="bg-white dark:bg-gray-800 shadow-lg rounded-lg overflow-hidden hover:shadow-xl transition-shadow duration-300">
          <router-link :to="{ name: 'book.show', params: { id: book.id } }">
            <div class="relative bg-gray-100 p-3 dark:bg-gray-700 flex justify-center items-center m-3 rounded-lg">
              <img :src="book.cover_image" alt="Book Image" class="h-52 object-cover rounded-lg" />
